...
class VAE(nn.Module):
	def __init__(self, nf=8, save_dir='', lr=1e-3, num_covariates=7, num_latents=32, device_name="auto", num_inducing_pts=6, mll_scale=10.0, l1_scale=1.0):
		...
		#for task
		self.xu_task = torch.linspace(-1.0, 4.0, self.inducing_pts).to(self.device)
		...
		#x trans
		self.xu_x = torch.linspace(-0.3, 0.3, self.inducing_pts).to(self.device)
        ...
        #y trans
		self.xu_y = torch.linspace(-0.4, 0.2, self.inducing_pts).to(self.device)
		...
		#z trans
		self.xu_z = torch.linspace(-1.0, 1.3, self.inducing_pts).to(self.device)
		...
		#rotational ones
		#xrot
		self.xu_xrot = torch.linspace(-0.02, 0.02, self.inducing_pts).to(self.device)
		...
		#yrot
		self.xu_yrot = torch.linspace(-0.01, 0.02, self.inducing_pts).to(self.device)
		...
		#zrot
		self.xu_zrot = torch.linspace(-0.004, 0.008, self.inducing_pts).to(self.device)
	...
	def project_latent(self, loaders_dict, save_dir, title=None, split=284):
