...
class VAE(nn.Module):
	def __init__(self, nf=8, save_dir='', lr=1e-3, num_covariates=7, num_latents=32, device_name="auto", task_init = '', num_inducing_pts=6, mll_scale=10.0, l1_scale=1.0, gp_cutoff=1e-4):
		...
		#adding gp_cutoff term
		self.gp_cutoff = gp_cutoff
	...
	def forward(self, ids, covariates, x, return_latent_rec=False):
		...
		for i in range(1,self.num_covariates+1):
			...
			#add it to gp loss
			#gp_loss += gp_mll
			#check out variance of predicted means
			#if it falls below a certain min, print it out and use only linear regressor!
			y_q_vars = torch.var(y_q)
			if y_q_vars <= self.gp_cutoff: #if post mean is flat ...
				#print(y_q_vars.detach().cpu().numpy(), i)
				task_var = covariates[:, i-1]
			else:
				#add residual prediction from GP to task variable
				task_var = covariates[:, i-1] + y_q
				gp_loss += gp_mll #add GP mll to loss ONLY IF using GP
			...
			#enforce small l1 regularization on all covariate maps
		...
		#adding GP losses & l1 regularization to VAE loss
	...
	def save_state(self, filename):
		...
		state['gp_cutoff'] = self.gp_cutoff
	...
	def load_state(self, filename):
		...
		self.gp_cutoff = checkpoint['gp_cutoff']
	...
	def plot_GPs(self, csv_file = '', save_dir = ''):
		...
		for i in range(len(regressors)):
			...
			#testing eleminating a GP if yq is flat
			yq_var = torch.var(yq)
			if yq_var <= self.gp_cutoff:
				#set posterior mean & inducing pts to zero
				yq = 0 * yq
				yu = 0 * yu
				#am leaving yvar as is right now
