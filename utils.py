...
def get_all_runs_data(feat_dirs, subj_idx, num_runs=4):
    """
    ...
    for i in range(num_runs):
        run_filt_data_path = os.path.join(feat_dirs[subjs[subj_idx]][j], 'filtered_func_data.nii.gz')
        assert os.path.exists(run_filt_data_path), 'Failed to find filtered data for run {}'.format(j)
        run_filtered_data = np.array(nib.load(run_filt_data_path).dataobj).reshape(-1, data_dims[3])
        all_runs_data.append(run_filtered_data)
    filtered_data = np.concatenate(all_runs_data, axis=1)
    return filtered_data

def get_all_runs_dms(feat_dirs, subj_idx, num_runs=4):
    """
    ...
    for i in range(num_runs):
        ...
        run_mat = utils.read_design_mat(run_mat_path)
        task_cols = run_mat[:, 0:3].reshape((data_dims[3], 3)) #am taking 3first cols which are the basic contrasts for EMERALD
        mot_cols = run_mat[:, -6:] #considers motion params are last 6 columns of DM. Double check that this is true...
        run_mat = np.concatenate((task_cols, mot_cols), axis=1)
        all_dms.append(run_mat)
