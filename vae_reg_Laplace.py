...
from torch.distributions import LowRankMultivariateNormal, Normal, Laplace, kl
...
class VAE(nn.Module):
	def __init__(self, nf=8, save_dir='', lr=1e-3, num_covariates=7, num_latents=32, device_name="auto", task_init = '', num_inducing_pts=6, mll_scale=10.0):
	...
	def forward(self, ids, covariates, x, return_latent_rec=False):
		...
		#get Laplace scale param -- this should be sigma/sqrt(2)
		sigma = torch.exp(-self.epsilon.unsqueeze(0).view(1, -1).expand(ids.shape[0], -1)).float()
		b = np.sqrt(0.5) * sigma
		obs_dist = Laplace(x_rec.float(), b)
