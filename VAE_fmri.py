"""
Created joining code pieces from:

- https://github.com/jackgoffinet/autoencoded-vocal-analysis/blob/master/ava/models/vae.py - Jack's VAE code
- https://github.com/pearsonlab/variational_autoencoder_fmri/blob/working_copy_rw/vae.py - Rachel's fMRI VAE
...
#uncomment if need to use matplotlib 3.0 & python3.5 version
#import matplotlib
#matplotlib.use('agg')
...
import os
import itertools
#did not clean up dependencies here

IMG_SHAPE = (41,49,35)
IMG_DIM = np.prod(IMG_SHAPE)

class VAE(nn.Module):
	""" Will add better documentation as I go...
	This is the same VAE class Jack's code version had
	Only it incorporates some methods from Rachel's fMRI version
	And has small adjustments - see comments
	"""
	def __init__(self, nf=8, save_dir='', lr=1e-3, z_dim=32, model_precision=10.0, device_name="auto"):
	...
	def _build_network(self):
		...
		Main modification here was adding layers fc31,32,33 and
		fc41,42,43 to Rachel's original architecture
		to yield an output in the form mu,u,d.
		This makes it easier to re-use several of Jack's methods
		"""
		# Encoder
		...
		#Decoder
		self.fc5 = nn.Linear(self.z_dim, 50)
	...
	def project_latent(self, loaders_dict, filename, title=None, split=2000):
		# Collect latent means.
		# for now, this is done for train only.
		# could add a for to create sep. plots for train and test
	...
	def reconstruct(self, input_volume, ref_nii):
		...
		with torch.no_grad():
			...
			nib.save(recon_nifti, 'reconstructed_data.nii') #add flexibility here for saving in a specific dir
