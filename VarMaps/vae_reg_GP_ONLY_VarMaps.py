...
class VAE(nn.Module):
	...
	def _build_network(self):
		...
		self.convt51 = nn.ConvTranspose3d(self.nf,1,3,1)
		self.convt52 = nn.ConvTranspose3d(self.nf,1,3,1)
		self.bnt1 = nn.BatchNorm3d(2*self.nf, track_running_stats=False)
		self.bnt3 = nn.BatchNorm3d(2*self.nf, track_running_stats=False)
		self.bnt51 = nn.BatchNorm3d(self.nf, track_running_stats=False)
		self.bnt52 = nn.BatchNorm3d(self.nf, track_running_stats=False)
	...
	def _get_layers(self):
		...
		return {'fc1':self.fc1, 'fc2':self.fc2, 'fc31':self.fc31,
                ...
                'bnt51':self.bnt51, 'bnt52':self.bnt52, 'conv1':self.conv1,'conv2':self.conv2,
                ...
                'convt51':self.convt51, 'convt52':self.convt52}
	...
	def forward(self, ids, covariates, x, return_latent_rec=False):
		...
		gp_loss = 0
		#set batch L1 regularization term to zero
		l1_reg = 0
		...
		x_rec_mean, x_rec_var = self.decode(zcat)
		...
		for i in range(1,self.num_covariates+1):
			...
			cons = torch.einsum('b,bx->bx', y_q, diff)
	...
	def plot_GPs(self, csv_file = '', save_dir = ''):
		...
		posterior mean +/- 2stds for a trained GPs
